...
def generate():
    ...
    add_dstu_heading(doc, "4.2 Сценарій ініціалізації auto/config та вимкнення інсталятора", level=2)
    add_dstu_paragraph(
        doc,
        ...
        "архітектура процесора — amd64, тип вихідного файлу — iso-hybrid. Параметр security вимкнено (оскільки в Devuan оновлення безпеки інтегровано в "
        "основний репозиторій). Через особливості структури репозиторіїв Devuan Excalibur, де відсутній сумісний завантажувач "
        "debian-installer за стандартним шляхом, параметр debian-installer було встановлено в значення false для уникнення "
        "помилок збірки 404. Це забезпечило побудову стабільного та чистого Live-образу, орієнтованого на роботу з оперативної пам'яті."
    )

    add_dstu_heading(doc, "4.3 Оновлений список пакетів та кастомізація середовища", level=2)
    add_dstu_paragraph(
        doc,
        "Списки пакетів для встановлення описуються у файлі config/package-lists/custom.list.chroot. Конфігурація містить: "
        "графічну оболонку XFCE 4 та дисплейний менеджер lightdm; базовий сервер відображення xorg; "
        "інструменти мережевої безпеки (nmap, curl, wget, tcpdump, network-manager); медіа-стек для систем відеоспостереження "
        "(mpv, pavucontrol, alsa-utils, va-driver-all, mesa-va-drivers); службові утиліти (sudo, btop, lm-sensors, openssh-server, thunar-volman, gvfs, fastfetch). "
        "Замість застарілого htop у збірку інтегровано сучасну консольну панель моніторингу btop, яка надає детальні "
        "динамічні графіки споживання ресурсів та температури процесорів."
    )
    add_dstu_paragraph(
        doc,
        "Для конфігурування кіоск-режиму у live-системі здійснено такі налаштування: "
        "1) у файлі config/includes.chroot/etc/lightdm/lightdm.conf активовано автоматичний вхід без пароля для користувача user; "
        "2) у файлі config/includes.chroot/etc/skel/.config/autostart/disable-sleep.desktop прописано команду xset s off -dpms "
        "для відключення енергозбереження та сну екрана (що критично для постів охорони, які мають вести моніторинг 24/7); "
        "3) у файлі config/includes.chroot/etc/skel/.config/autostart/kiosk-app.desktop налаштовано автоматичний запуск утиліти "
        "btop у повноекранному терміналі xfce4-terminal при кожному вході; "
        "4) створено автостарт-заглушки xfce-polkit.desktop та polkit-gnome-authentication-agent-1.desktop з прапором Hidden=true "
        "для усунення помилки дублювання агентів авторизації PolicyKit на старті системи."
    )
    ...
    add_dstu_paragraph(
        ...
        "фінальний гібридний образ devuan-security-amd64-amd64.hybrid.iso. Отриманий файл розміром 951 МБ копіюється з "
        "віртуальної машини на хост-систему за допомогою безпечного протоколу SCP."
    )
    add_dstu_paragraph(
        doc,
        "На рисунку 3.2 показано результат виведення розміру та права доступу створеного ISO-образу на хост-машині."
    )
    ...
    add_dstu_heading(doc, "6.1 Верифікація автоматичного входу та кіоск-режиму btop", level=2)
    add_dstu_paragraph(
        doc,
        ...
        "відбулося в автоматичному режимі (Live CD). Графічний інтерфейс XFCE запустився успішно з автоматичною "
        "авторизацією без запиту пароля. Після завантаження робочого столу на весь екран автоматично відкрилася консоль "
        "моніторингу btop, демонструючи успішну роботу кіоск-режиму та відсутність візуальних помилок авторизації."
    )
    add_dstu_paragraph(
        doc,
        "На рисунку 3.3 показано завантажений робочий стіл XFCE 4 нашої кастомної операційної системи в режимі кіоску."
    )

    add_screenshot(doc, os.path.join(SCREENSHOTS_DIR, "4.1_devuan_live_desktop.png"), "3.27", "Робочий стіл XFCE 4 кастомного дистрибутиву Devuan Security OS з автозапуском btop")

    add_dstu_heading(doc, "6.2 Налаштування вибіркової персистентності та перевірка ініціалізації", level=2)
    add_dstu_paragraph(
        doc,
        "Для забезпечення збереження відеозаписів та логів при роботі системи повністю в оперативній пам'яті (stateless), "
        "було підключено другий віртуальний диск /dev/sdb1, відформатований в ext4 та позначений міткою persistence. "
        "У файлі конфігурації persistence.conf прописано шлях /home/user/recordings, що дозволило реалізувати вибіркове "
        "монтування типу bind. Це забезпечує збереження записів камер на фізичному диску, тоді як усі налаштування ОС "
        "залишаються захищеними в RAM. Перевірка процесу ініціалізації PID 1 підтвердила використання sysvinit (init)."
    )
    add_dstu_paragraph(
        doc,
        "На рисунку 3.4 зображено термінал із виведенням інформації про систему, перевіркою PID 1 та успішним монтуванням розділу записів."
    )

    add_screenshot(doc, os.path.join(SCREENSHOTS_DIR, "4.2_devuan_sysvinit_proof.png"), "3.28", "Перевірка процесу ініціалізації та монтування диска збереження записів")
